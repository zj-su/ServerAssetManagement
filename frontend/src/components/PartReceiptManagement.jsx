...
const PartReceiptManagement = () => {
  ...
  const fetchReceipts = async () => {
    try {
      ...
      alert('获取入库单列表失败: ' + (err.response?.data?.detail || err.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteReceipt = async (receiptId) => {
    if (!window.confirm('确定要删除这个入库单吗？只有空入库单才能删除。')) {
      return;
    }
  ...
  const revokeReceiptItem = async (itemId) => {
    if (!window.confirm('确定要撤销这个资产吗？只能撤销两天内录入的资产。')) {
      return;
    }
                      ...
                      {selectedReceipt.items.map((item) => (
                        <tr key={item.id}>
                          ...
                          <td style={{ padding: '8px', border: '1px solid #ddd' }}>
                            <button 
                              onClick={() => revokeReceiptItem(item.id)}
                              style={{ padding: '4px 8px', fontSize: '12px', backgroundColor: '#dc3545', color: 'white' }}
                            >
                              撤销
                            </button>
